...
def _call_gemini_api(prompt_text: str, api_key: str = "", max_tokens: int = 256) -> str:
    """
    Send the scene-scoring prompt to local Ollama instead of Google Gemini.

    The function keeps the original Gemini name so the rest of MovieShort AI
    does not need to know that the LLM is running locally.
    """
...
def _check_gemini_key(api_key: str = "") -> dict:
    """
    Check the local Ollama server instead of checking a Gemini API key.

    The function keeps its original name because the GUI/provider system
    still identifies this provider as "gemini".
    """
    base = getattr(
        config,
        "OLLAMA_BASE_URL",
    ...
    try:
        ...
        if model not in model_names:
            ...
            if not any(
                name.split(":")[0] == base_model
                for name in model_names
            ):
                return {
                    "ok": False,
                    "error": f"Ollama: модель {model} не установлена",
                }

        # Actually test inference so the GUI check confirms that the model
        # can respond, not merely that Ollama is running.
        resp = httpx.post(
            f"{base}/api/chat",
            json={
                "model": model,
